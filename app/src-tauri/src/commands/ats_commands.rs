...
use crate::db::{DbPool, ResumeRepository, JobRepository, GeneratedDocumentRepository, GeneratedResume, GeneratedCoverLetter, ApplicationRepository, JobApplication, BackgroundTaskRepository};
...
#[tauri::command]
pub async fn generate_ats_resume_queued(
    app: AppHandle,
    job_id: String,
    resume_id: String,
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let generated_resume_json = json!({
            ...
            "optimized": {
                ...
                "experience_bullets": optimized.experience_bullets,
                "ats_score": optimized.ats_score,
        ...
        let generated = match GeneratedDocumentRepository::create_resume(
            &pool,
            &resume.user_id,
            &job.id,
            &resume.id,
            ...
            Some(
                generated_resume_json["optimized"]["skills"]
                    .as_array()
                    .map(|skills| skills.iter().filter_map(|s| s.as_str()).collect::<Vec<_>>().join(", "))
                    .unwrap_or_default()
            ),
            Some(generated_resume_json["optimized"]["experience_bullets"].to_string()),
        ).await {
