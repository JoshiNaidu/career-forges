    ...
    pub async fn search(
        ...
        query_rows(
            ...
            SELECT
                ...
                mime_type, parsed_content, hash, is_default,
                created_at, updated_at, master_resume_json,
                ats_score, ats_strengths, ats_weaknesses, ats_recommendations
            FROM resumes
            WHERE user_id = ?1 AND deleted_at IS NULL
              AND (filename LIKE ?2 OR parsed_content LIKE ?2 OR master_resume_json LIKE ?2)
            ORDER BY created_at DESC
            ",
            (user_id.to_string(), pattern),
            Self::map_row,
        )
        .await
    }
