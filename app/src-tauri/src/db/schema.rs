...
pub fn get_migrations() -> Vec<crate::db::migration::Migration> {
    vec![
        ...
        migration!(
            "003_background_tasks",
            "
            CREATE TABLE IF NOT EXISTS background_tasks (
                id TEXT PRIMARY KEY,
                task_type TEXT NOT NULL CHECK(task_type IN ('resume_generation','cover_letter_generation','job_fetch','resume_parsing','interview_analysis','download','update','other')),
                status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','running','completed','failed','cancelled','retry')),
                label TEXT NOT NULL,
                entity_type TEXT,
                entity_id TEXT,
            ...
            CREATE INDEX IF NOT EXISTS idx_bg_tasks_entity ON background_tasks(entity_type, entity_id);

            INSERT OR IGNORE INTO settings (id, key, value, data_type, description, created_at, updated_at) VALUES
            ('11', 'default_resume_template', 'classic', 'string', 'Default ATS resume template', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('12', 'resume_storage_location', '', 'string', 'Custom resume storage location (empty = Documents/CareerForges)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('13', 'notify_new_jobs', 'true', 'boolean', 'Notify when new matching jobs are found', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('14', 'notify_resume_ready', 'true', 'boolean', 'Notify when an ATS resume is ready', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('15', 'notify_interview_finished', 'true', 'boolean', 'Notify when interview analysis completes', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('16', 'notify_update_available', 'true', 'boolean', 'Notify when an app update is available', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('17', 'notify_task_completed', 'false', 'boolean', 'Notify when any background task completes', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('18', 'font_scale', '100', 'number', 'UI font scale percentage (100 = default)', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('19', 'high_contrast', 'false', 'boolean', 'Enable high contrast mode', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
            ('20', 'developer_mode', 'false', 'boolean', 'Enable developer tools and debug console', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
            "
        ),
    ]
}
