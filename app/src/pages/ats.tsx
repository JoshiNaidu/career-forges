...
import type { Resume, GeneratedResume, GeneratedCoverLetter, Job, BackgroundTask } from "@/lib/db/models";
...
export default function ATSPage() {
  ...
  const [queueTasks, setQueueTasks] = useState<Record<string, BackgroundTask>>({});
  const [activeJobs, setActiveJobs] = useState<Job[]>([]);
  const [selectedQueueJobId, setSelectedQueueJobId] = useState<string>("");
  ...
  const loadData = async () => {
    ...
    if (users.length > 0) {
      ...
      if (!selectedQueueJobId && allJobs.length > 0) {
        setSelectedQueueJobId(allJobs[0].id);
      }
  ...
  // Load active queue tasks and listen for updates
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      try {
        const users = await db.listUsers();
        if (users.length > 0) {
          const tasks = await db.listRecentBackgroundTasks(20);
          const taskMap: Record<string, BackgroundTask> = {};
          tasks
            .filter(t => t.taskType === "resume_generation" && (t.status === "pending" || t.status === "running"))
            .forEach(t => { taskMap[t.id] = t; });
          setQueueTasks(taskMap);
        }
      } catch (e) {
        console.error("[ATS] Failed to load background tasks", e);
      }

      // Listen for task updates
      unlistenFn = await listen<string>("background-task-updated", async (event) => {
        const taskId = event.payload;
        try {
          const updated = await db.getBackgroundTask(taskId);
          if (updated) {
            setQueueTasks(prev => {
              const next = { ...prev };
            ...
            // If completed, reload history
            if (updated.status === "completed") {
              await loadData();
            }
          }
        } catch (e) {
          console.error("[ATS] Failed to fetch updated task", e);
        }
      });

      // Listen for resume-ready events
      const unlistenResumeReady = await listen<{ taskId: string; resumeId: string; jobTitle: string }>("resume-ready", async (event) => {
        await loadData();
        setQueueTasks(prev => {
          const next = { ...prev };
          delete next[event.payload.taskId];
          return next;
        });
      });
  ...
  const activeTaskCount = Object.values(queueTasks).filter(
    t => t.status === "pending" || t.status === "running"
  ).length;

  const handleQueueGeneration = useCallback(async () => {
    if (!selectedResumeId || !selectedQueueJobId) return;
    try {
      const taskId = await db.generateAtsResumeQueued(selectedQueueJobId, selectedResumeId);
      const task = await db.getBackgroundTask(taskId);
      if (task) {
        setQueueTasks(prev => ({ ...prev, [taskId]: task }));
      }
    } catch (e) {
      console.error("[ATS] Queue generation failed", e);
    }
  }, [selectedResumeId, selectedQueueJobId]);
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-white/5 bg-white/[0.02] p-8 space-y-6">
              ...
              {selectedResume && activeJobs.length > 0 && (
                <div className="space-y-3 rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between">
                    <label className="text-sm font-medium text-[var(--muted)] flex items-center gap-2">
                      <Clock className="h-4 w-4 text-orange-400" />
                      Queue ATS Resume Generation
                    </label>
                    {activeTaskCount > 0 && (
                      <span className="flex items-center gap-1 text-[10px] font-bold text-orange-400 bg-orange-400/10 px-2 py-0.5 rounded-full">
                        ...
                        {activeTaskCount} in queue
                      </span>
                    )}
                  </div>
                  <select
                    className="w-full rounded-lg border border-white/5 bg-[var(--surface)] px-3 py-2 text-sm outline-none focus:border-orange-500/50"
                    value={selectedQueueJobId}
                    onChange={(e) => setSelectedQueueJobId(e.target.value)}
                  >
                    {activeJobs.map(job => (
                  ...
                  <button
                    ...
                    disabled={!selectedResumeId || !selectedQueueJobId || activeTaskCount > 0}
                    className="flex w-full items-center justify-center gap-2 rounded-lg border border-orange-500/30 bg-orange-500/10 py-2.5 text-sm font-bold text-orange-400 transition hover:bg-orange-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {activeTaskCount > 0 ? (
                      <>
                      ...
                      <>
                        <Zap className="h-4 w-4" />
                        Generate Optimized Resume (Queued)
                      </>
                    )}
                  </button>
                </div>
              ...
              {Object.keys(queueTasks).length > 0 && (
                <div className="space-y-2 rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  <h4 className="text-xs font-bold uppercase tracking-widest text-[var(--muted)]">Active Tasks</h4>
                  {Object.values(queueTasks).map(task => (
                    <div key={task.id} className="flex items-center gap-3 rounded-lg bg-white/[0.02] p-3">
                      {task.status === "running" ? (
                      ...
                      <span className="text-xs font-medium text-[var(--muted)]">{task.progress}%</span>
                    </div>
                  ))}
                </div>
              )}
