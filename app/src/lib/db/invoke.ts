...
export const resumeInvoke = {
  ...
  async download(id: string): Promise<string> {
    return invokeWithRetry('download_resume', { id });
  },

  async search(userId: string, query: string): Promise<Resume[]> {
    return invokeWithRetry('search_resumes', { userId, query });
  },

  async getStorageInfo(): Promise<StorageInfo> {
    return invokeWithRetry('get_storage_info', {});
  },

  async openResumesFolder(): Promise<void> {
    return invokeWithRetry('open_resumes_folder', {});
  },
};
...
export const atsInvoke = {
  ...
  async generateResumeQueued(jobId: string, resumeId: string): Promise<string> {
    return invokeWithRetry('generate_ats_resume_queued', { jobId, resumeId });
  },
...
// ============ Background Task Invoke Wrappers ============

export const backgroundTaskInvoke = {
  async create(
    taskType: string,
    label: string,
    entityType?: string,
    ...
    return invokeWithRetry('db_create_background_task', {
      taskType,
      label,
      entityType,
      entityId,
  ...
  async get(id: string): Promise<BackgroundTask | null> {
    return invokeWithRetry('db_get_background_task', { id });
  },

  async listByStatus(status: string): Promise<BackgroundTask[]> {
    return invokeWithRetry('db_list_background_tasks_by_status', { status });
  },

  async listRecent(limit: number): Promise<BackgroundTask[]> {
    return invokeWithRetry('db_list_recent_background_tasks', { limit });
  },

  async listByEntity(entityType: string, entityId: string): Promise<BackgroundTask[]> {
    return invokeWithRetry('db_list_background_tasks_by_entity', { entityType, entityId });
  },

  async updateStatus(id: string, status: string): Promise<void> {
    return invokeWithRetry('db_update_background_task_status', { id, status });
  },

  async updateProgress(id: string, progress: number, currentStep?: number): Promise<void> {
    return invokeWithRetry('db_update_background_task_progress', { id, progress, currentStep });
  },

  async cancel(id: string): Promise<void> {
    return invokeWithRetry('db_cancel_background_task', { id });
  },

  async retry(id: string): Promise<void> {
    return invokeWithRetry('db_retry_background_task', { id });
  },

  async delete(id: string): Promise<void> {
    return invokeWithRetry('db_delete_background_task', { id });
  },

  async countByStatus(status: string): Promise<number> {
    return invokeWithRetry('db_count_background_tasks_by_status', { status });
  },
};
...
export const dbInvoke = {
  ...
  backgroundTask: backgroundTaskInvoke,
};
