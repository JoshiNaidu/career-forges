...
class DatabaseService {
  ...
  async downloadResume(id: string): Promise<string> {
    return dbInvoke.resume.download(id);
  }

  async searchResumes(userId: string, query: string): Promise<Resume[]> {
    return dbInvoke.resume.search(userId, query);
  }

  async getStorageInfo(): Promise<StorageInfo> {
    return dbInvoke.resume.getStorageInfo();
  }

  async openResumesFolder(): Promise<void> {
    return dbInvoke.resume.openResumesFolder();
  }
  ...
  async generateAtsResumeQueued(job_id: string, resume_id: string): Promise<string> {
    return dbInvoke.ats.generateResumeQueued(job_id, resume_id);
  }
  ...
  async createBackgroundTask(
    ...
    return dbInvoke.backgroundTask.create(taskType, label, entityType, entityId, totalSteps);
  }

  async getBackgroundTask(id: string): Promise<BackgroundTask | null> {
    return dbInvoke.backgroundTask.get(id);
  }

  async listBackgroundTasksByStatus(status: BackgroundTask['status']): Promise<BackgroundTask[]> {
    return dbInvoke.backgroundTask.listByStatus(status);
  }

  async listRecentBackgroundTasks(limit: number = 50): Promise<BackgroundTask[]> {
    return dbInvoke.backgroundTask.listRecent(limit);
  }

  async listBackgroundTasksByEntity(entityType: string, entityId: string): Promise<BackgroundTask[]> {
    return dbInvoke.backgroundTask.listByEntity(entityType, entityId);
  }

  async updateBackgroundTaskStatus(id: string, status: BackgroundTask['status']): Promise<void> {
    return dbInvoke.backgroundTask.updateStatus(id, status);
  }

  async updateBackgroundTaskProgress(id: string, progress: number, currentStep?: number): Promise<void> {
    return dbInvoke.backgroundTask.updateProgress(id, progress, currentStep);
  }

  async cancelBackgroundTask(id: string): Promise<void> {
    return dbInvoke.backgroundTask.cancel(id);
  }

  async retryBackgroundTask(id: string): Promise<void> {
    return dbInvoke.backgroundTask.retry(id);
  }

  async deleteBackgroundTask(id: string): Promise<void> {
    return dbInvoke.backgroundTask.delete(id);
  }

  async countBackgroundTasksByStatus(status: BackgroundTask['status']): Promise<number> {
    return dbInvoke.backgroundTask.countByStatus(status);
  }
}
